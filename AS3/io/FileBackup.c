...
struct directory* newDirectory(char* path, char* name, int type){
...
void InitLLFS()
{
    ...
    free(buffer);
    folders[0] = newDirectory("/", "root", 1);  // Initialize root directory
...
{
    if (type ==0){
        printf("start to create flat file: %s\n", name);
        ...
        for(int i =1; i<MAX_FOLDER_NUM; i++){
            if(folders[i]==NULL){
                folders[i] = newDirectory(path, name, 1);
                if(folders[i]==NULL){
                    return;
                }
                break;
