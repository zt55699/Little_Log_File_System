...
void Loading(FILE* disk){
    ...
    if(strlen(InodeMap[0].name)==0)
        strcpy(InodeMap[0].name,"root");
    free(buffer);
...
int Check_child(int currenr_folder, char* child_name){
    ...
    for(int i =1; i <MAX_PER_FOLDER; i++){
        ...
        if(files[ Find_folder_index(InodeMap[currenr_folder].name) ]->inode_index[i] == child)
            return child;
    }
    return -1;
}

//return 0 if not find parent, else return parent's inode#
int Check_parent(int current_folder){
    for(int i =0; i <MAX_FOLDER_NUM; i++){
        if(files[i] == NULL)
            continue;
        for(int j =1; j<MAX_PER_FOLDER; j++){
            if(files[i]->inode_index[j]== current_folder)
                return files[i]->inode_index[0];
        }
    }
    return 0;
}
...
void Print_childs(int current_folder){
    //printf (" Inodemap[%d] find %s\n",current_folder, InodeMap[current_folder].name);
    int folderNum =Find_folder_index(InodeMap[current_folder].name);
    if (folderNum ==-1){
        printf("ERROR: No %s in memory\n",InodeMap[current_folder].name );
    }
    int count = 0;
    for(int i=1; i<MAX_PER_FOLDER;i++){
        if(files[folderNum]->names[i]!= NULL){
            printf("%s [%dbytes]\n", files[folderNum]->names[i], InodeMap[files[folderNum]->inode_index[i]].size);
            count++;
        }
    }
    if(count == 0){
        printf("[This is an empty folder.]\n");
    }
}
